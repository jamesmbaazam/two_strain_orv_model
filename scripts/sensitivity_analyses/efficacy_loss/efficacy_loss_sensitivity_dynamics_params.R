...
#' Vaccine escape from reduced efficacy against variant, assuming perfect 
#' efficacy agains the wild type

dynamics_params_efficacy_loss_sensitivity <- data.frame(
    R0w = R0_w,
    R0m = R0_m_original,
    beta_w = R0_w / IP_w,
    ...
    vax_efficacy_m = seq(0.5, 0.8, 0.1) # perfect vaccine efficacy against variant
)