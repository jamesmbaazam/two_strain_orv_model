...
efficacy_loss_sensitivity_analysis_all_results <- bind_rows(baseline_analysis_results, efficacy_loss_sensitivity_analysis_results) 
...
efficacy_loss_sensitivity_analysis_pop_rescaled <- efficacy_loss_sensitivity_analysis_df %>%
    mutate(across(.cols = c(total_cases, peak_prevalence), 
                  .fns = ~ .x*target_pop
                  ),
           variant_emergence_day = as_factor(variant_emergence_day), 
           vax_efficacy_w = as_factor(vax_efficacy_w),
...
outbreak_size_efficacy_loss_isocline_df <- efficacy_loss_sensitivity_analysis_pop_rescaled %>% 
...
# Isoclines ----
# Outbreak size ====
outbreak_size_efficacy_loss_isocline_sensitivity <- ggplot(outbreak_size_efficacy_loss_isocline_df %>% 
                                                               filter(vax_efficacy_m %in% c(1, 0.6)), 
                                 aes(x = vax_coverage, 
                                     y = min_speed, 
    ...
    labs(#title = paste('Cumulative cases threshold <= 1000 at various NPI intensity levels'), 
        ...
        linetype = 'Vaccine efficacy against variant'
    ) +
...
ggsave(outbreak_size_efficacy_loss_isocline_sensitivity,
       filename = 'outbreak_size_efficacy_loss_isocline_sensitivity.png',
       path = git_plot_path,
       width = 23.76,
       height = 17.86,
       units = 'cm')
...
ggsave(outbreak_size_efficacy_loss_isocline_sensitivity,
       filename = 'outbreak_size_efficacy_loss_isocline_sensitivity.eps',
...
peak_prevalence_efficacy_loss_isocline_df <- efficacy_loss_sensitivity_analysis_pop_rescaled %>% 
    filter(variant_emergence_day %in% c(1, 61, 121, 151, max_time),
           npi_intensity %in% c(0.0, 0.1, 0.2, 0.3), 
           peak_prevalence <= 300) %>% 
