...
cp_simulation_table <- bind_cols(intervention_params_expanded, cp_dynamics_parameters_expanded) %>% 
    ...
    filter(variant_emergence_day %in% c(61, 121), 
           sigma_w %in% c(0.5, 1)
           )
...
# save the simulation
saveRDS(object = cp_sensitivity_summaries, file = "./model_output/sensitivity_analyses/cross_protection/cp_subset_sensitivity_analysis_summaries.rds")
