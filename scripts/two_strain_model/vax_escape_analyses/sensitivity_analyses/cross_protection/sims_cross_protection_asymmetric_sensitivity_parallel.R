...
dynamics_params_cp_asymmetric <- dynamics_params_cp_asymmetric_sensitivity
...
intervention_params_expanded <- intervention_params_table %>%
    slice(rep(row_number(), times = nrow(dynamics_params_cp_asymmetric)))
...
#'data frames
cp_asymmetric_dynamics_parameters_expanded <- dynamics_params_cp_asymmetric %>% 
...
cp_asymmetric_simulation_table <- bind_cols(intervention_params_expanded, cp_asymmetric_dynamics_parameters_expanded) %>% 
...
n_sims <- nrow(cp_asymmetric_simulation_table)
...
cp_asymmetric_sensitivity_summaries <- foreach(
    i = 1:num_of_jobs,
    .combine = rbind,
    ...
    sim_subset <- cp_asymmetric_simulation_table %>%
...
# save the simulation
saveRDS(object = cp_asymmetric_sensitivity_summaries, file = "./model_output/sensitivity_analyses/cross_protection/cp_asymmetric_sensitivity_analysis_summaries.rds")
