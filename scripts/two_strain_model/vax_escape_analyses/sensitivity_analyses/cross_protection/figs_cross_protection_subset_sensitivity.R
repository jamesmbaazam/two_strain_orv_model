...
total_cases_thresholds <- c(0.00001, 0.01, 0.1, 0.15, 0.2, 0.5)
...
for (cases_threhold in seq_along(total_cases_thresholds)) {
    ...
    outbreak_size_cp_isocline_sensitivity <- ggplot(outbreak_size_cp_isocline_df, 
        ...
        scale_y_continuous(breaks = seq(1, 10, 0.5), labels = seq(1, 10, 0.5)) +
        scale_color_viridis_d(option = 'viridis') +
        labs(title = 'Sensitivity to cross protection assumptions',
             subtitle = paste0('Strategies with cumulative cases up to ', 
    ...
    file_name <-  paste0('./figures/sensitivity_analyses/cross_protection/cp_sensitivity_subset_isocline_outbreak_size', total_cases_thresholds[cases_threhold]*100,'.png')
}

#Save the files to a pdf
pdf(width = 12, file = "./figures/sensitivity_analyses/cross_protection/cp_subset_isoclines_outbreak_size.pdf")
...
# Peak prevalence ====
peak_prevalence_thresholds <- c(0.000006, total_cases_thresholds/100)
...
for (peak_prevalence_threhold in seq_along(peak_prevalence_thresholds)) {
    ...
    peak_prevalence_cp_isocline <- ggplot(peak_prevalence_cp_isocline_df, 
        ...
        scale_color_viridis_d(option = 'viridis') +
        labs(title = 'Sensitivity to cross protection assumptions', 
             subtitle = paste0('Strategies with peak prevalence up to ', 
    ...
    file_name <-  paste0('./figures/sensitivity_analyses/cross_protection/cp_subset_sensitivity_peak_prevalence_isocline_', 
                         peak_prevalence_thresholds[[peak_prevalence_threhold]]*100,
                         '.png'
    )
}
...
#Save the files to a pdf
pdf(width = 12, file = "./figures/sensitivity_analyses/cross_protection/cp_subset_isoclines_peak_prevalence.pdf")
