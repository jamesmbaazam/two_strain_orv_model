...
#All scenarios
vax_escape_sim_table <- vax_escape_sim_config_table 
...
n_sims <- nrow(vax_escape_sim_table)
...
vax_escape_dynamics <- foreach(i = 1:num_of_jobs, 
                                          .combine = rbind,
                                          ...
                                          .errorhandling = 'remove') %dopar% 
    {
        ...
        sim_subset <- vax_escape_sim_table %>% 
...
vax_escape_dynamics_final <- vax_escape_dynamics %>% 
    mutate(R0m = R0_m, 
           vax_efficacy_w = dynamics_params$vax_efficacy_w, 
           vax_efficacy_m = dynamics_params$vax_efficacy_m, 
           cross_protection = dynamics_params$sigma_w
           )
