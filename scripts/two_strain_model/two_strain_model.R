...
#'
#' @examples
extract_ts_model_summaries <- function(dynamics_df) {
...
simulate_raw_dynamics <- function(pop_inits, dynamics_parms,
  ...
  # Model run ####
  sim_results <- as.data.frame(lsoda(pop_inits, dt, two_strain_model,
    parms = sim_parms,
  ...
  if (get_summaries) {
    model_summaries <- extract_ts_model_summaries(sim_results_with_controls)
...
simulate_vax_escape_dynamics <- function(pop_inits, dynamics_parms,
                                         control_parms, max_time,
                                         dt, events_table,
  ...
  if (get_summaries) {
    model_summaries <- extract_vax_escape_mod_summaries(sim_results_with_controls)
    return(model_summaries)
  } else {
    return(sim_results_with_controls)
  }
...
run_sim_all_ts_model <- function(sim_table, get_summaries) {
  res <- sim_table %>%
    rowwise() %>%
    do({
      with(
        .,
        simulate_raw_dynamics(
          pop_inits = pop_inits,
...
#'
#' @examples
run_sim_all_vax_escape_model <- function(sim_table, get_summaries) {
  res <- sim_table %>%
    rowwise() %>%
    do({
      with(
        .,
        simulate_vax_escape_dynamics(
          pop_inits = pop_inits,
          dynamics_parms = dynamics_params,
          control_parms = .,
          max_time = max_time,
